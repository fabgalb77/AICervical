...
def predict(img):
    ...
    n_locations_local = 4
    warmup_epoch = 20
    model = CoordRegressionNetwork(arch, n_locations_global, n_locations_local, pretrained, kernel_size, n_ch, n_blocks).to(device)
      
    model.load_state_dict(torch.load('./src/model/weights_local.pth', map_location = device))
    ...
    coords_original_pred_local = np.concatenate((coords_original_pred_local, coords_original_pred[-5:]))
...
if __name__ == '__main__':
    if check_password():
        ...
        if file:
            images, parameters, sp = predict(file)
            #st.image(img_dc)
            print('T1 slope: {}\n'.format(sp.t1_slope()))
            print('C2-C7: {}\n'.format(sp.c2c7()))
